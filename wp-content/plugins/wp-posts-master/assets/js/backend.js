jQuery(document).ready(function($) {
    ...
    $('.wpp_choose_design').click(function() {
        if (!confirm("Apply this design? It'll overwrite your existing design.")) {
            return;
        }
        layout_id = parseInt($(this).attr('rel'));
        thumbnail_position = $(this).data('thumbnail');
        $(".wpp_choose_design").parent().removeClass('wpp_design_selected');
        $('input[name="layout_type"]').val(layout_id);
        $('.wpp_source_code').val($(this).find('.wpp_cs_source_code').val());
        $(this).parent().addClass('wpp_design_selected');
        if (thumbnail_position == 'left') {
            $("select[name='layout_post_setting[thumb_position]']").val('left');
        } else if (thumbnail_position == 'right') {
            $("select[name='layout_post_setting[thumb_position]']").val('right');
        }

    });
    /********/
    $("body").on('click', "input[name='save_source_code']", function() {
        $("input[name='layout_post_setting[source_code]']").val($('.wpp_source_code').val());
        $('.switch_tabs form').submit();
    });
    $("body").on('click', "input[name='reset_source_code']", function() {

        $('.wpp_source_code').val($('.wpp_design_selected').find('.wpp_choose_design').find('.wpp_cs_source_code').val());
    });

    $("body").on('change', "input[name='layout_post_setting[pagination_style]']", function() {
        var value = $(this).val();
        $('input[name="layout_post_setting[per_page]"]').parent().parent().show();
        $('.wpp_carousel_settings').closest('.form-group ').hide();
        if (value == 'carousel') {
            $('input[name="layout_post_setting[per_page]"]').parent().parent().hide();
            $('.wpp_carousel_settings').closest('.form-group ').show();
        }
    });
    ...
    $("body").on('change', "input[name='rule_match[post_type]']", function() {
        var value = $(this).val();
        var selected_value = $('input[name="entityID"]').val();
        $('.all_taxonomies').parent().addClass('hiderow');
        $('.all_terms').parent().addClass('hiderow');
        if (!$(this).is(':checked') || !value || value == 'all' || value == 'page' || value == '') {

            return;
        }
        jQuery.ajax({
            type: "POST",
            url: wpp_js_lang.ajax_url,
            data: {
                action: 'wpp_ajax_call',
                operation: 'get_taxonomies',
            ...
            success: function(data) {
                $('.all_taxonomies').html(data);
                $('.all_taxonomies').parent().removeClass('hiderow');
                $("input[name='rule_match[category_taxonomy]']").trigger('change');
    ...
    $("body").on('change', "input[name='rule_match[category_taxonomy]']", function() {
        var value = $(this).val();
        var selected_value = $('input[name="entityID"]').val();

        if (!$(this).is(':checked') || !value || value == '') {
            $('.all_terms').parent().addClass('hiderow');
            return;
        }
        jQuery.ajax({
            ...
            success: function(data) {
                $('.all_terms').html(data);
                $('.all_terms').parent().removeClass('hiderow');
            }
    ...
    /********/
    $("body").on('click', ".wpp_refresh", function() {
        $('.posts_preview').addClass("wpp_loading");
        var data = $('.switch_tabs form').serializeArray();
        var post_data = {};
        $.each(data, function(key, name_value) {
            post_data[name_value.name] = name_value.value;
        });
        post_data['source_code'] = $('.wpp_source_code').val();
        post_data['action'] = 'wpp_ajax_call';
        post_data['operation'] = 'wpp_load_template';
        post_data['nonce'] = wpp_js_lang.nonce;
        post_data['selected_value'] = selected_value;
        var selected_value = $('input[name="entityID"]').val();
        jQuery.ajax({
            type: "POST",
            url: wpp_js_lang.ajax_url,
            ...
            beforeSend: function() {},
            success: function(data) {
                $('.posts_preview').removeClass("wpp_loading");
                $('.posts_preview').html(data.html);
            }
    ...
    $(".wpp_check_backup").click(function() {
        var wpp_bid = confirm("Overwrite existing google maps database?.");
        if (wpp_bid == true) {
    ...
    $(".wpp_search_input").keyup(function() {
        map_id = $(this).attr("rel");
        $(".wpp_locations_listing[rel='" + map_id + "']").addClass("wpp_loading");
        wpp_filter_locations(map_id, 1);
    });

    $(".wpp_toggle_container").click(function() {
        $(".wpp_toggle_main_container").toggle("slow");
        if ($(this).text() == "Hide") {
            $(this).text("Show");
        } else {
            $(this).text("Hide");
    ...
    $(".wpp_mcurrent_loction").click(function() {
        wpp_get_current_location();
    });
    ...
    var maptable = $('#wpp_google_map_data_table').dataTable({
        "lengthMenu": [
    ...
    var route_maptable = $('#wpp_google_map_route_data_table').dataTable({
        "lengthMenu": [
            [10, 25, 50, 100, 200, 500, -1],
            [10, 25, 50, 100, 200, 500, "All"]
        ],
...
function send_icon_to_map(imagesrc, target) {
    wpp_image_id = jQuery("#" + target);
    jQuery(wpp_image_id).find('.remove_image').show();
    jQuery(wpp_image_id).find('img.selected_image').attr('src', imagesrc);
    var img_hidden_field = jQuery(wpp_image_id).find('.choose_image').data('target');
    jQuery(wpp_image_id).find('input[name="' + img_hidden_field + '"]').val(imagesrc);
