 ...
 *  HOW TO USE:
    ...
    RMD_Validator_Handler::set_fields(array(
        'input_name' => array( 'label'=> 'Full Name', 'validation' => array('email')),
    )); 
    $response = RMD_Validator_Handler::run(); 
 
 */ 
 
class RMD_Email_Validator extends RMD_Validator 
{
	...
	protected function validate_field($field_name, $field_label, $field_message) 
	{
		if(isset($_POST[$field_name]) && !empty($_POST[$field_name])) { 
            if (filter_var($_POST[$field_name], FILTER_VALIDATE_EMAIL) === false) {  
	...
	{
		return '<p><strong>'.$field_label.'</strong> requires a valid email address.</p>';
	}  
