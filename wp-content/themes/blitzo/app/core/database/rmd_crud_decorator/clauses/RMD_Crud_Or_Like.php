...
/**
 *	The RMD_Crud_Or_Like will manage to make a sql query with a like clause using OR operator
 */
/***
 * 	HOW TO USE
 *
	...
	$crud = new RMD_Crud_Or_Like($crud, array('ID'=>1)); 
	$response = $crud->get();
 
 */
class RMD_Crud_Or_Like extends RMD_Crud_Clause
{
	...
	private function set_like($like_arr = array()) 
	{
		if(!empty($like_arr)) {
			if(!is_array($like_arr)) {
				exit('Error: RMD_Crud_Or_Like requires an array as its second parameter.');
			}
		} else {
			exit('Error: RMD_Crud_Or_Like requires an array as its second parameter.');
		}
