...
/**
 *	The RMD_Crud_Where will manage to make a sql query with a where clause using AND operator
 */
/***
 * 	HOW TO USE
 *
	...
	$crud = new RMD_Crud_Where($crud, array(
		array('field1','=','value1'),
		array('field2','=','value2')
	)); 
	$response = $crud->get();
 ...
 */
class RMD_Crud_Where extends RMD_Crud_Clause
{
	 ...
	 */
	public function set_where($where_arr = array()) 
	{
		if(!empty($where_arr)) {
			if(!is_array($where_arr)) {
				exit('Error: RMD_Crud_Where requires an multi dimentional array as its second parameter.');
			}
		} else {
			exit('Error: RMD_Crud_Where requires an multi dimentional array as its second parameter.');
		}
